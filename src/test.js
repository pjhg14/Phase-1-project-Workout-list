...
//reference
//JSON Server
//use _embed to include related children resources
    //...localhost:3000/users?_embed=lists
    //returns users with related lists (from db1.json in data)
    //use _expand to do the same but with parent resources

//When adding a new parent object to json
    //Create Parent component
    //Create empty children components

// fetch("localhust3000/users", {
//     method: "POST",
//     headers: {
//         "content-type": "application/json"
//     },
...
//     .then(resp => resp.json())
//     .then(newUser => {
//         //make sure that parent has empty version of all children 
//         newUser.lists = []
//
//         //show info on user
//     })
...
//Filter/Search
    //use embed with the desired collections and the name of the parent
    //i.e localhost:3000/users?_embed=lists&username=[insert user name]
    //i.e.e. localhost:3000/users?_embed=lists&username=Eric
        //returns => {
        //     "id": 1,
        //     "userID": 1,
        //     "name": "Grocery",
        //     "items": ["Chicken", "Beef", "Sandwiches"]
        // },
        // {
        //     "id": 2,
        //     "userID": 1,
        //     "name": "Potential DJ names",
        //     "items": ["Grandmaster Slamma Jamma","Smooth E"]
        // }
...
//Overall:
    //adding items:
        //let addedItems = [...itemList, newItem]
    //deleting items
        //let filteredList = itemList.filter(item => item.id !== target.id)
        //obj.itemList = filterList


//for project:
//exercise: list of known exercises(standalone)
//user: user of the app(standalone)
//routines: List(array) of routines for each user (dependant of usere)

//For phase2
    //create: spread
    //let copy = [...original, new]
    //setState(copy)